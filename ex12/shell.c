...
// this func checking the validity of the syntax
// input: char *str - string.
// return: true or false.
Bool isValidJob(char *str) {
	int i;
	char *jobStr = str + 1;
	int len = strlen(jobStr);	
...
//this func find job by ID
//input: jobId & job list
//return: pointer to the specific job.
struct job* findJobById(int jobId, struct jobSet* jobList) {
...
int runCommand(struct job newJob, struct jobSet * jobList, 
    ...
    //            job->text);
 	// while statusString is one of the {Stopped, Running}    	
    	...
    	while(job)
    	{
    		statusString = job->stoppedProgs > 0 ? "Stopped" : "Running";
    	...
    	if(isValidJob(newJob.progs[0].argv[1]) == false) {
    		return 1;
    	}

    	char *jobNumStr = newJob.progs[0].argv[1] + 1;
    	jobNum = atoi(jobNumStr);

    	// else find the job in the job list
		//using findJobById.
		job = findJobById(jobNum, jobList);
